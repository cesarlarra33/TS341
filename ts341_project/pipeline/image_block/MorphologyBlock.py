...
class MorphologyBlock(ProcessingBlock):
    ...
    def __init__(
        self, operation: str = "closing", kernel_size: int = 5, iterations: int = 1
    ):
        """
        Args:
    ...
    def process(
        ...
        if self.operation == "erode":
            result.frame = cv2.erode(frame, self.kernel, iterations=self.iterations)
        elif self.operation == "dilate":
            result.frame = cv2.dilate(frame, self.kernel, iterations=self.iterations)
            ...
            raise ValueError(f"Opération morphologique inconnue: {self.operation}")
